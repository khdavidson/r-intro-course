...
# One of the most common suite of tools is a whole group of packages that reside in the package "tidyverse". 
# We won't use these packages today, but we will install and load them so that we are ready for next week (if haven't already). 
...
data.xl <- read.xlsx("r-intro-week1_calibration.xlsx", sheet = 2)      # sheet index = sheet number. 
...
# LOOKING AT DATA #
#=================#
# Now we have our data read in, recall this is a data frame. Recall: table or a two-dimensional data structure with a mix of variable types (numeric, factor, character, etc.)

##### MULTIPLE VARIABLES/DATAFRAME AT ONCE
...
#####
  ...
  # We can see if we look through we have a few types of variables:
  ...
  # E.g., scroll down to "hpe_startdate" - it has assigned dates to be Factors - BECAUSE they don't have a year attached! 
  ...
  # Note the difference between variable types in how R imported the .csv file vs. the .xlsx file. 
  # Depending on the analysis you wanted to do, this could be an issue. 
...
# Can use the above '$' symbol combined with syntax we learned before w/ embedded vector method we learned before 
